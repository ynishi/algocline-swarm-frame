...
describe("swarm_frame.sequence", function()
    it("runs handlers in order and returns the last response", function()
        ...
        local h = frame.sequence({
            function() table.insert(calls, "a") return "DONE path=a" end,
            function() table.insert(calls, "b") return "DONE path=b" end,
            function() table.insert(calls, "c") return "DONE path=c" end,
        })
    ...
    it("short-circuits on a non-DONE verdict", function()
        ...
        local h = frame.sequence({
            function() table.insert(calls, "a") return "DONE path=a" end,
            function() table.insert(calls, "b") return "BLOCKED reason=stop" end,
            function() table.insert(calls, "c") return "DONE path=c" end,
        })
        local ctx = { state = frame.state_new() }
        local resp = h(ctx)
    ...
    it("rejects opts missing the handlers array via lshape in strict mode", function()
        ...
        local ok, err = pcall(function()
            frame.sequence({ cp_key = "x" }, "swarm_frame.sequence", "strict")
        end)
        ...
        local ok2, err2 = pcall(lshape_check.assert,
            { cp_key = "x" },
            "SwarmFrame.SequenceOpts",
            "swarm_frame.sequence")
