-- host_seam_spec.lua — Tests for the M.host DI seam and 2-step JSON chain.
--
-- Tests the following chain paths (both are acceptance criteria):
--   (a) M.host explicit injection — custom encode/decode used
--   (b) _G.alc.json_encode/decode mock — algocline-engine VM path
--
-- The legacy (c) vendored pure_json fallback (v0.9.0 and earlier) was
-- removed in v0.10.0; swarm_frame now requires either M.host or
-- _G.alc.json_* and raises an error otherwise.
...
describe("host seam", function()
    ...
    -- ─── (c) no-host raises ─────────────────────────────────────────────────
    --
    -- v0.10.0 removed the vendored pure_json / dkjson / cjson fallback.
    -- With M.host=nil and _G.alc absent the chain must raise rather than
    -- silently fall through to a vendored provider.

    describe("missing host raises", function()
        local saved_alc
        ...
        it("raises when M.host=nil and _G.alc is absent", function()
            _G.alc = nil
            expect(function() frame.json_encode({}) end).to.fail()
            expect(function() frame.json_decode("{}") end).to.fail()
        end)

        it("error message mentions algocline runtime", function()
            _G.alc = nil
            expect(function() frame.json_encode({}) end).to.fail.with("algocline runtime")
        end)
    end)
