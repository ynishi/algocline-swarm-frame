-- update_dispatch_record_spec.lua — validation, happy path, error propagation.
--
...
local M = require("swarm_state_method")
...
-- Build a mocked `alc` table whose `state.show` returns the seeded state
-- and whose `state.set_dispatched` records the last write into `last`.
...
describe("swarm_state_method.run", function()
    describe("entry-point validation", function()
        ...
        it("errors when alc is not a table", function()
            local ok, err = pcall(M.run, { action = "update_dispatch_record", alc = "bad" })
            expect(ok).to.equal(false)
        ...
        it("errors on unknown action", function()
            ...
            local ok, err = pcall(M.run, { action = "rollback_step", alc = mock })
            expect(ok).to.equal(false)
    ...
    describe("update_dispatch_record validation", function()
        it("errors when namespace is missing", function()
            local mock = make_alc_show_returns({ data = {} })
            local ok, err = pcall(M.run, { action = "update_dispatch_record", alc = mock, key = "k", update = {} })
            expect(ok).to.equal(false)
        ...
        it("errors when key is missing", function()
            local mock = make_alc_show_returns({ data = {} })
            local ok, err = pcall(M.run, { action = "update_dispatch_record", alc = mock, namespace = "ns", update = {} })
            expect(ok).to.equal(false)
        ...
        it("errors when update is missing", function()
            local mock = make_alc_show_returns({ data = {} })
            local ok, err = pcall(M.run, { action = "update_dispatch_record", alc = mock, namespace = "ns", key = "k" })
            expect(ok).to.equal(false)
        ...
        it("errors when update is not a table", function()
            ...
            local ok, err = pcall(
                M.run,
                { action = "update_dispatch_record", alc = mock, namespace = "ns", key = "k", update = "bad" }
            )
            expect(ok).to.equal(false)
            expect(contains(err, "update")).to.equal(true)
        end)
        ...
        it("errors when alc.state.set_dispatched is missing", function()
            local mock = { state = { show = function() return { data = {} } end } }
            local ok, err = pcall(M.run, {
                action = "update_dispatch_record",
                alc = mock,
                namespace = "ns",
        ...
        it("errors when alc.state.show returns string and alc.json_decode is missing", function()
            local mock = {
                state = {
            ...
            expect(ok).to.equal(false)
            expect(contains(err, "alc.json_decode is unavailable")).to.equal(true)
        end)

        it("errors when alc.json_decode fails on the show payload", function()
            local mock = {
                state = {
            ...
            expect(ok).to.equal(false)
            expect(contains(err, "alc.json_decode failed")).to.equal(true)
        end)
    end)

    describe("update_dispatch_record happy path", function()
        it("decodes JSON string returned by alc.state.show via alc.json_decode", function()
            local set_calls = 0
            local last_value = nil
            local mock = {
                state = {
                    show = function() return '{"data":{"status":"pending","actor":"old"}}' end,
                ...
                json_decode = function(s)
                    -- minimal stub that returns a fixed parse for the seeded payload
                    expect(s).to.equal('{"data":{"status":"pending","actor":"old"}}')
        ...
        it("merges patch into state.data and writes back via set_dispatched", function()
            local seed = { data = { status = "pending", actor = "old" }, identity = { task_id = "k" } }
            ...
            -- updated_fields is sorted; assert both fields present.
            ...
            -- set_dispatched was called with the merged state.
            ...
            -- non-overlapping existing field preserved.
        ...
        it("returns empty updated_fields when update table is empty", function()
            local seed = { data = { x = 1 } }
            local mock, last = make_mock_alc(seed)
            local result = M.run({
                action = "update_dispatch_record",
            ...
            -- still writes back (idempotent) — last call recorded.
            expect(last.calls).to.equal(1)
...
describe("swarm_state_method.meta", function()
    it("declares v0.1.0", function()
        expect(M.meta.version).to.equal("0.1.0")
        expect(M.VERSION).to.equal("0.1.0")
    end)

    it("declares category=state_method", function() expect(M.meta.category).to.equal("state_method") end)

    it("declares alc_shapes_compat", function() expect(type(M.meta.alc_shapes_compat)).to.equal("string") end)
end)
